...
export const addNewProductToBD = async ({ description, title, price, image_url }: ICreateNewProductParams) => {
  console.log('Data to add: ', description, title, price, image_url);
  const  client = new Client(dbOptions);
  await client.connect();
  await createIfNotExists(client);
  ...
  try {
    const { rows: product } = await client.query(
      'insert into products (description , price, title, image_url) values ($1, $2, $3, $4)',
      [description, price, title, image_url]
    );
    console.log(product);
    return product;
